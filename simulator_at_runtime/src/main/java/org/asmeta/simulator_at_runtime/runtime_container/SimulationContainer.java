...
import org.asmeta.simulator_at_runtime.runtime_composer.AsmetaModel;
import org.asmeta.simulator_at_runtime.runtime_composer.ModelCreationException;
import org.asmeta.simulator_at_runtime.runtime_simulator.AsmetaSservice;
import org.asmeta.simulator_at_runtime.runtime_simulator.IdNotFoundException;
import org.asmeta.simulator_at_runtime.runtime_simulator.InfoAsmetaService;
...
public class SimulationContainer implements IModelExecution, IModelAdaptation {
	...
	public void rollback(int id) {
		try {
			...
			// TODO: Da scrivere su LOG, l'esecuzione � corretta
			return;
		}
	}
...
//	//Rimane comunque il problema che se allo scadere non ha finito il programma restituir� run timed out con rollback della sim
...
//                    e.printStackTrace();}
//    		}
//    		while (!routTO.getTimeoutFlag()) {	//se non ho ancora rOut vuol dire che non ha ancora finito, devo aspettare altrimenti non funziona pi� nulla successivamente
//    			try {
//    				Thread.sleep(10);	
//        		} catch (InterruptedException e) {
//                    e.printStackTrace();}
...
//			{	//OLDtodo se il rollback � gi� stato fatto lo fa di nuovo perch� l'equals di mystate non esiste (provato a sistemare con variabile rolledbackQ)
//				try {
//					if (!rolledbackQ)
//						printRollback(asmS.getSimulatorTable().get(id).getContSim(), asmS.rollback(id));
